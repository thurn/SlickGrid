/***
 *
 * (c) 2009 Michael Leibman (michael DOT leibman AT gmail.com)
 * All rights reserved.
 *
 *
 * TODO:
		...
		function updateRow(row) {
			...
			$(rowsCache[row]).find(".slick-cell").each(function(i) {
				...
				if (row == currentRow && i == currentCell && currentEditor)
					currentEditor.setValue(data[row][m.field]);
				else
					this.innerHTML = data[row] ? m.formatter(row, i, data[row][m.field], m, data[row]) : "";
			});
